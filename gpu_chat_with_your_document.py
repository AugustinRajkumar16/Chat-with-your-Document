...
def get_gpu_utilization() -> str:
    try:
        # Run the nvidia-smi command to get GPU utilization
        gpu_util = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=utilization.gpu", "--format=csv,noheader,nounits"]
        )
        gpu_util = gpu_util.decode("utf-8").strip()  # Decode byte output to string
        # logger.info("GPU utilize: %s", gpu_util)  # Correct logging format
        return gpu_util
    except subprocess.CalledProcessError as e:
        logger.info("Error retrieving GPU utilization: %s", str(e))  # Correct logging format
        return f"Error: {str(e)}"

# Function to update GPU utilization on Streamlit interface
def display_gpu_utilization():
    gpu_util = get_gpu_utilization()  # Get current GPU utilization
    logger.info(f"GPU Utilization: {gpu_util}%")  # Correct logging format
...
def create_vector_db(file_upload) -> Chroma:
    ...
    # If the model supports .to() method for moving to GPU, you can move it manually
    if device == "cuda" and hasattr(embeddings, 'to'):
        display_gpu_utilization()
        logger.info("Moving to GPU using embeddings")
...
def process_question(question: str, vector_db: Chroma, selected_model: str) -> str:
    ...
    llm = ChatOllama(model=selected_model, device="cuda")  # Use GPU for ChatOllama
...
def main() -> None:
    ...
    # Update GPU utilization periodically
    display_gpu_utilization()  # Display initial GPU utilization
        ...
        with col1:
            ...
            if file_upload:
                if st.session_state["vector_db"] is None:
                    ...
                    time.sleep(2)  # Simulating some delay (for example, document processing)
                    # You can check the GPU utilization during this long process
                    for _ in range(5):  # Let's simulate a task and update GPU utilization every time
                        display_gpu_utilization()
                        time.sleep(5)
